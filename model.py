...
from transformers.models.bart.modeling_bart import BartEncoder, BartEncoderLayer, BartDecoderLayer, BartDecoder
import copy
...
class JointEncoder(BartEncoder):
    def __init__(self, config, embed_tokens=None, patch_size=None):
        ...
        self.block = nn.ModuleList([BartEncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def forward(
        ...
        batch_size, seq_length = input_shape # bs, 64
...
class BartForMultimodalGeneration(BartForConditionalGeneration):
    ...
    def __init__(self, config: BartConfig, patch_size):
        ...
        self.decoder = BartDecoder(decoder_config, self.shared)
